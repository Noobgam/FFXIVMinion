...
function e_movetogate:execute()
	...
	if (mapid == 399 and Player.localmapid == 478) then
		...
		local questGate = {x = 73.259323120117, y = 205, z = 143.04707336426, h = -0.52216768264771}
		local defaultGate = {x = 147.0463, y = 207, z = 115.8594, h = 0.9793}
		local useQuestGate = QuestCompleted(1658)
		if section ~= 0 then
			if section == 1 then
				ml_global_information.ShowInformation(GetString("Destination is hinterlands section 1."), 5000)
				pos = useQuestGate and questGate
			else
...
function c_useaethernet:evaluate(mapid, pos)
	...
	if (nearestAethernet and bestAethernet) then
		...
		if (destMapID ~= Player.localmapid and bestAethernet.Invisible and bestAethernet.territory == destMapID) then
			compareBestDistance = 0
		end
		aethernetDetour = IsNull(nearestDistance, math.huge) + compareBestDistance
	end
...
function PickNetworkCrystalApproachPos(rawPos, fallbackPos, maxCrystalDistance)
	...
	local function addCandidate(probePos, snapDistance)
		...
		for _, existing in ipairs(candidates) do
			if (math.distance3d(existing, meshPt) < 1) then
				return
			end
		end
		table.insert(candidates, meshPt)
	end

	addCandidate(fallbackPos, 4)
	addCandidate(rawPos, 15)

	local myPos = Player and Player.pos
	if (table.valid(myPos)) then
		...
		for _, radius in ipairs(radii) do
			for _, angleOffset in ipairs(angleOffsets) do
				...
				addCandidate({
					x = rawPos.x + math.sin(angle) * radius,
					y = rawPos.y,
					z = rawPos.z + math.cos(angle) * radius,
				}, 8)
			end
		end
	end
	...
	local directCrystalDistance = math.distance3d(myPos, rawPos)
	local maxReasonablePath = math.max(6, directCrystalDistance * 3)
	local pathThreshold = math.max(40, maxReasonablePath + 20)
	local bestAcceptable = nil
	local bestAcceptableScore = math.huge
...
function c_mount:evaluate()
	-- Airborne landing is handled by ml_navigation after the general lock checks.
	...
	if (table.valid(gotoPos)) then
		...
		if (not needsMount and not (ffnav.isascending or ffnav.isdescending or IsMounting())) then
			local within = dismountDistance > 0 and dist2d <= dismountDistance
				and dist3d <= (dismountDistance + 3)
			if (within and Player.ismounted and not ml_task_hub:CurrentTask().remainMounted) then
				local task = ml_task_hub:CurrentTask()
				...
				if (task.interact and task.interact ~= 0) then
					...
					doDismount = not interactable or interactDistance < (dismountDistance + 5)
				else
					doDismount = true
				end
