		...
		if (ffxivminion.gameRegion == 1) then
			...
			if (table.valid(serviceAccountList)) then
				if (SelectConversationLine(FFXIV_Login_ServiceAccount)) then
					ml_global_information.Await(500, 5000, function()
						return MGetGameState() ~= FFXIV.GAMESTATE.MAINMENUSCREEN
					end)
				end
			else
			...
			if (IsControlOpen("CharaSelect")) then
				if (not IsControlOpen("_CharaSelectWorldServer")) then
					...
					if (serverControl) then
						serverControl:Open()
						ml_global_information.Await(1000, 10000, function()
							return IsControlOpen("_CharaSelectWorldServer")
						end)
					end
				else
								...
								if (gChocoItemString == itemdetails.name) then
									...
									if (item and item:IsReady() and companion and companion.alive) then
										...
										if (MissingBuffs(companion, buffString)) then
											...
											ml_global_information.Await(1500, function()
												return not MIsMoving()
											end)
...
function ffxivminion.SetMainVars()
	...
	if not SettingsUUID.Login.ServiceAccount then
		if (Settings.Global.FFXIV_Login_ServiceAccounts and string.valid(uuid) and Settings.Global.FFXIV_Login_ServiceAccounts[uuid]) then
			...
			SettingsUUID.Login.ServiceAccount = ffxivminion.GetSetting("FFXIV_Login_ServiceAccount", 1)
		end
	end
	FFXIV_Login_ServiceAccount = SettingsUUID.Login.ServiceAccount

	---LoginCharacter
	if not SettingsUUID.Login.Character then
		if (Settings.Global.FFXIV_Login_Characters and string.valid(uuid) and Settings.Global.FFXIV_Login_Characters[uuid]) then
			...
			SettingsUUID.Login.Character = ffxivminion.GetSetting("FFXIV_Login_Character", 0)
		end
	end
...
function ffxivminion.HandleInit()
	...
	if (meshfilelist) then
		for i, file in spairs(meshfilelist, function(file, a, b)
...
function ffxivminion.FillFoodOptions(availableonly)
	...
	if (table.valid(foods)) then
		for id, item in spairs(foods, function(item, a, b)
			return item[a].name < item[b].name
		end) do
				...
				if (ffxivminion.GUI.main.visible) then
					...
					if (modeChanged) then
						...
						if (string.valid(uuid)) then
							if (Settings.FFXIVMINION.gBotModes == nil) then
								Settings.FFXIVMINION.gBotModes = {}
							end
			...
			if (ffxivminion.GUI.settings.visible) then
				...
				if (tabindex == 3) then
					...
					for i, abrev in pairsByKeys(fighters) do
						...
						GUI:SameLine(0, 10)
						if (count % 6) == 0 and count ~= table.size(fighters) then
							GUI:NewLine()
						end
						count = count + 1
					end
					...
					for i, abrev in pairsByKeys(crafters) do
						...
						GUI_Capture(GUI:InputInt("##" .. abrev, _G[str], 0, 0), str)
						GUI:SameLine(0, 10)
						if (count % 6) == 0 and count ~= table.size(crafters) then
							GUI:NewLine()
						end
						count = count + 1
					end
					...
					for i, abrev in pairsByKeys(gatherers) do
						...
						GUI_Capture(GUI:InputInt("##" .. abrev, _G[str], 0, 0), str)
						GUI:SameLine(0, 10)
						if (count % 6) == 0 and count ~= table.size(gatherers) then
							GUI:NewLine()
						end
						count = count + 1
					end
				...
				if (tabindex == 5) then
					...
					GUI_Capture(GUI:Checkbox(GetString("assistMode"), gChocoAssist), "gChocoAssist");
					GUI:SameLine()
					GUI_Capture(GUI:Checkbox(GetString("grindMode"), gChocoGrind), "gChocoGrind");
					GUI:SameLine()
				...
				if (tabindex == 6) then
					...
					GUI_DrawIntMinMax(GetString("Avoid HP"), "gAvoidHP", 1, 10, 0, 100, function()
						ffxivminion.SaveClassSettings("gAvoidHP", gAvoidHP)
					end)
					GUI_DrawIntMinMax(GetString("Rest HP"), "gRestHP", 1, 10, 0, 100, function()
						ffxivminion.SaveClassSettings("gRestHP", gRestHP)
					end)
					GUI_DrawIntMinMax(GetString("Rest MP"), "gRestMP", 1, 10, 0, 100, function()
						ffxivminion.SaveClassSettings("gRestMP", gRestMP)
					end)
					GUI_DrawIntMinMax(GetString("Potion HP"), "gPotionHP", 1, 10, 0, 100, function()
						ffxivminion.SaveClassSettings("gPotionHP", gPotionHP)
					end)
					GUI_DrawIntMinMax(GetString("Potion MP"), "gPotionMP", 1, 10, 0, 100, function()
						ffxivminion.SaveClassSettings("gPotionMP", gPotionMP)
					end)
					GUI_DrawIntMinMax(GetString("Flee HP"), "gFleeHP", 1, 10, 0, 100, function()
						ffxivminion.SaveClassSettings("gFleeHP", gFleeHP)
					end)
					GUI_DrawIntMinMax(GetString("Flee MP"), "gFleeMP", 1, 10, 0, 100, function()
						ffxivminion.SaveClassSettings("gFleeMP", gFleeMP)
					end)
					...
					GUI_DrawIntMinMax(GetString("Avoid HP##Eureka"), "gEurekaAvoidHP", 1, 10, 0, 100, function()
						ffxivminion.SaveClassSettings("gEurekaAvoidHP", gEurekaAvoidHP)
					end)
					GUI_DrawIntMinMax(GetString("Rest HP##Eureka"), "gEurekaRestHP", 1, 10, 0, 100, function()
						ffxivminion.SaveClassSettings("gEurekaRestHP", gEurekaRestHP)
					end)
					GUI_DrawIntMinMax(GetString("Rest MP##Eureka"), "gEurekaRestMP", 1, 10, 0, 100, function()
						ffxivminion.SaveClassSettings("gEurekaRestMP", gEurekaRestMP)
					end)
					GUI_DrawIntMinMax(GetString("Potion HP##Eureka"), "gEurekaPotionHP", 1, 10, 0, 100, function()
						ffxivminion.SaveClassSettings("gEurekaPotionHP", gEurekaPotionHP)
					end)
					GUI_DrawIntMinMax(GetString("Flee HP##Eureka"), "gEurekaFleeHP", 1, 10, 0, 100, function()
						ffxivminion.SaveClassSettings("gEurekaFleeHP", gEurekaFleeHP)
					end)
					GUI_DrawIntMinMax(GetString("Flee MP##Eureka"), "gEurekaFleeMP", 1, 10, 0, 100, function()
						ffxivminion.SaveClassSettings("gEurekaFleeMP", gEurekaFleeMP)
					end)
				...
				if (tabindex == 8) then
					...
					GUI_DrawIntMinMax("Service Account Index (0-n)", "FFXIV_Login_ServiceAccount", 1, 1, 0, 15,
							function()
								SettingsUUID.Login.ServiceAccount  = FFXIV_Login_ServiceAccount
							end
					)
					if (GUI:IsItemHovered()) then
						GUI:SetTooltip(GetString("When multi account are tagged in single SE-account"))
					end

					GUI_DrawIntMinMax("Character Index (0-n)", "FFXIV_Login_Character", 1, 1, 0, 15,
							function()
								SettingsUUID.Login.Character = FFXIV_Login_Character
								ffxivminion.loginvars.charSelected = false
							end
					)
					GUI:PopItemWidth()
...
function ml_global_information.DrawHelper()
	-- Helper Window
	local gamestate = MGetGameState()
	if (gamestate == FFXIV.GAMESTATE.INGAME) then
...
invalid name or haven't chosen one.")
				...
				if (tabs.tabs[1].isselected) then
					...
					if activeACRProfile then
						...
						GUI:SameLine();
						GUI:Text(tostring(gSkillProfile))
					end
					GUI:Text("Current Task: ");
					GUI:SameLine();
					GUI:Text(FFXIV_Core_ActiveTaskName)
					GUI:Text("Eorzea Time: ");
					GUI:SameLine();
					GUI:Text(FFXIV_Common_EorzeaTime)
					GUI:Separator()
					...
					GUI:SameLine();
					GUI:Text(tostring(Player.flying.canflyinzone))
					GUI:Text("Mount Can Fly: ");
					GUI:SameLine();
					GUI:Text(tostring(Player.mountcanfly))
					GUI:Text("Is Flying: ");
					GUI:SameLine();
					GUI:Text(tostring(Player.flying.isflying))
					GUI:SameLine(200);
					GUI:Text("Is Diving: ");
					GUI:SameLine();
					GUI:Text(tostring(Player.diving.isdiving))
					GUI:Text("Is Position Locked: ");
					GUI:SameLine();
					...
					if gBotMode == "questMode" then
						...
						if (TableSize(questList) > 0) then
							...
							GUI:SameLine();
							GUI:Text(TableSize(questList))
						end

						GUI:Text("Profile : ");
						GUI:SameLine();
						GUI:Text(gQuestProfile)
						if gCurrQuestID ~= "" then
							GUI:Text("Quest ID : ");
							GUI:SameLine();
							GUI:Text(gCurrQuestID)
						end
						if gQuestStepType ~= "" then
							GUI:Text("Quest Task : ");
							GUI:SameLine();
							GUI:Text(gQuestStepType)
						end
						if gCurrQuestStep ~= "" then
							GUI:Text("Quest Step Index : ");
							GUI:SameLine();
							GUI:Text(gCurrQuestStep)
						end
						if gCurrQuestObjective ~= "" then
							GUI:Text("Quest Objective : ");
							GUI:SameLine();
							GUI:Text(gCurrQuestObjective)
						end
						if gCurrQuestStep ~= "" then
							GUI:Text("Quest Step : ");
							GUI:SameLine();
...
function ml_global_information.DrawInformationPopup()
	-- Helper Window
	local gamestate = MGetGameState()
	if (gamestate == FFXIV.GAMESTATE.INGAME) then
